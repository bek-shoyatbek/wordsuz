...
export class AuthController {
  ...
  async adminLogin(
    @Body() body: LoginDto,
  ): Promise<BaseResponse<{ email: string; accessToken: string }>> {
    const { email, accessToken } = await this.authService.loginAsAdmin(
      body.email,
      body.password,
    );
    return {
  ...
  async verify(
    ...
    const { email, accessToken } = await this.authService.verify(
  ...
  > {
    const { email, accessToken, refreshToken } = await this.authService.login(
      body.email,
      body.password,
    );
    return {
